...
class Game:
    ...
    def roll(self, roll):
        print("%s is the current player" % self.players.get_player_name(self.current_player))
        print("They have rolled a %s" % roll)

        if self.in_penalty_box[self.current_player] and roll % 2 == 0:
            print("%s is not getting out of the penalty box" % self.players.get_player_name(self.current_player))
            self.is_getting_out_of_penalty_box = False
        else:
            if roll % 2 != 0  and self.in_penalty_box[self.current_player]:
                print("%s is getting out of the penalty box" % self.players.get_player_name(self.current_player))
            ...
            print(self.players.get_player_name(self.current_player)+ \
                        '\'s new location is ' + \
    ...
    def was_correctly_answered(self):
        if self.in_penalty_box[self.current_player]:
            if self.is_getting_out_of_penalty_box:
                ...
                if self.current_player == self.players.length(): self.current_player = 0
    ...
    def wrong_answer(self):
        ...
        print(self.players.get_player_name(self.current_player) + " was sent to the penalty box")
